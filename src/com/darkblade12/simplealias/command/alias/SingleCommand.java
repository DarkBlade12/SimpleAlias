...
@CommandDetails(name = "single", params = "<name> <command>", description = "Creates a new alias for a single command", permission = Permission.CREATE_COMMAND, infiniteParams = true)
public final class SingleCommand implements ICommand {
	...
	public void execute(CommandHandler handler, CommandSender sender, String label, String[] params) {
		...
		if (manager.hasAlias(name)) {
			...
			actions.clear();
			actions.add(new CommandAction("ExecuteSingleCommand", new HashSet<String>(), new HashSet<String>(), new HashSet<String>(), new HashMap<Integer, String>(), 0, false, command, Executor.SENDER, false));
			List<String> executionOrder = alias.getExecutionOrder();
			executionOrder.clear();
			executionOrder.add("ExecuteSingleCommand");
			try {
				alias.save();
